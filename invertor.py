...
class ImageToLatentCodeInvertor():
    def __init__(self, E, Gs, batch_size=4, number_of_iters=100):
        ...
        self.wp = None
        self.x = None
        x_255, x_rec_255 = self.build_graph()
        self.configure_optimization(x_255, x_rec_255)
    ...
    def build_graph(self):
        self.x = tf.placeholder(tf.float32, shape=self.input_shape, name='real_image')
        x_255 = (tf.transpose(self.x, [0, 2, 3, 1]) + 1) / 2 * 255
        self.wp = tf.get_variable(shape=self.latent_shape, name='latent_code')
        self.x_rec = self.Gs.components.synthesis.get_output_for(self.wp, randomize_noise=False)
        x_rec_255 = (tf.transpose(self.x_rec, [0, 2, 3, 1]) + 1) / 2 * 255
        # Initialization of optimization as encoder output
        w_enc = self.E.get_output_for(self.x, is_training=False)
        wp_enc = tf.reshape(w_enc, self.latent_shape)
        self.setter = tf.assign(self.wp, wp_enc)
        return x_255, x_rec_255

    def configure_optimization(self, x_255, x_rec_255):
        ...
        # pixel-domain loss
        loss_pix = tf.reduce_mean(tf.square(self.x - self.x_rec), axis=[1, 2, 3])
        # domain regularizer
    ...
    def image_to_latent_code(self, images):
        ...
        for img_idx in range(0, number_of_images, self.batch_size):
            ...
            # Run encoder.
            self.sess.run([self.setter], {self.x: batch})
            ...
            for step in range(1, self.num_iterations):
                self.sess.run(self.train_op, {self.x: batch})
